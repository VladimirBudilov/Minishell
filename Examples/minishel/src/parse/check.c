...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/30 21:23:55 by armartir          #+#    #+#             */
/*   Updated: 2023/04/30 21:23:56 by armartir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_meta_s(t_minishell *shell, char *line, size_t *sym_c, int *index)
{
	char	last_meta[3];
	int		slice_size;

	ft_bzero(last_meta, 3);
	slice_size = check_slice((line + (*index)),
			SINGLE_SEPARATORS, DOUBLE_SEPARATORS);
	if (slice_size)
	{
		if (!ft_strlen(last_meta) && !(*sym_c)
